...
describe("LiquidityExamples", () => {
  ...
  before(async () => {
    ...
    const so = 50n * 10n ** 18n
  })

  it.only("mintNewPosition", async () => {
    const daiAmount = 100n * 10n ** 18n
    const usdcAmount = 100n * 10n ** 6n
    await dai
      .connect(accounts[0])
    ...
    await liquidityExamples
      .connect(accounts[0])
      .mintNewPosition(daiAmount,usdcAmount)
  ...
  it("collectAllFees", async () => {
    ...
    console.log(`dai ${await dai.balanceOf(liquidityExamples.address)}`)
    console.log(`usdc ${await usdc.balanceOf(liquidityExamples.address)}`)
    console.log(`dai acc${await dai.balanceOf(accounts[0].address)}`)
    console.log(`usdc acc ${await usdc.balanceOf(accounts[0].address)}`)
  })
})